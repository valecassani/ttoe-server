...
export const checkGame = gameId => {
    return connection('games').where({
        id: gameId
    }).first().select('id', 'maxUsers', 'winner');
}
...
export const resetGame = async gameId => {
    await connection('moves').where({ gameId }).del();
    await connection('games').where({id: gameId}).update({currentPlayer: 0, winner: null});
...
export const addMove = async move => {
    ...
    if (checkWinner(game)) {
        await connection('games').where({id: move.gameId}).update({ winner: currentPlayer })
        game = await getGame(move.gameId)
    } else {
        newCurrentPlayer = (currentPlayer + 1) % maxUsers;
        await connection('games').where({id: move.gameId}).update({ currentPlayer: newCurrentPlayer });
    }
